...
#else
#define ESP_XMODEM_RECEIVER_TASK_SIZE 4096
#endif
#define ESP_XMODEM_RECEIVER_TASK_PRIO 11
    ...
    for (i = 0; i < p->cycle_max_retry; i++) {
        ...
        if (esp_xmodem_read_data(receiver, p->cycle_timeout_ms) == 0) {
            ...
            for (;;) {
                ...
                if (ret != ESP_OK) {
                    fail_count ++;
                    ESP_LOGI(TAG, "err_code is 0x%x", esp_xmodem_get_error_code(receiver));
...
esp_err_t esp_xmodem_receiver_start(esp_xmodem_handle_t receiver)
{
    ...
    if (receiver->role != ESP_XMODEM_RECEIVER) {
        ESP_LOGE(TAG, "Now is receiver mode,please set role to ESP_XMODEM_SERVER");
        return ESP_ERR_INVALID_ARG;
    }
