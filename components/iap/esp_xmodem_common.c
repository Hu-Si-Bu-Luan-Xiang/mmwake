...
#else
#define ESP_XMODEM_HANDLE_TASK_SIZE 4096
#endif
#define ESP_XMODEM_HANDLE_TASK_PRIO 12

static void esp_xmodem_stop_task(void *pvParameters)
{
    vTaskDelay(500/portTICK_PERIOD_MS); //for main task delete itself
    xTaskHandle process_handle = (xTaskHandle) pvParameters;
    if (process_handle) {
...
esp_xmodem_handle_t esp_xmodem_init(const esp_xmodem_config_t *config, void *transport_config)
{
    ...
    p->cycle_max_retry = !config->cycle_max_retry ? 5 : config->cycle_max_retry;
...
esp_err_t esp_xmodem_stop(esp_xmodem_t *handle)
{
    ...
    if (xTaskCreate(esp_xmodem_stop_task, "xmodem_stop", ESP_XMODEM_HANDLE_TASK_SIZE, handle->process_handle, ESP_XMODEM_HANDLE_TASK_PRIO, NULL) != pdPASS) {
